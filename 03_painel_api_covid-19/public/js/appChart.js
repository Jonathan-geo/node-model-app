...
function dadosDataAtual(dados){

    //RETORNA A DATA ATUAL
            //FUNCAO QUE REMOVE ITEM NO JSON BUSCANDO POR VALOR(KEY)
    function removerPela(meuJSON, chave, valor){
        meuJSON = meuJSON.filter(function(jsonObject) {
            return jsonObject[chave] != valor;
        });
        return meuJSON
    }
            //REMOVENDO no DATASET OS ITEM EM QUE REGIAO É VAZIO
    //console.log(removerPela(csvArray, "regiao",""));
    
        //REMOVENDO no DATASET OS ITEM EM QUE REGIAO É VAZIO E RETORNANDO A DATA DA ULTIMA LINHA 
    //console.log(removerPela(csvArray, "regiao","")[csvArray.length-2].data);
    
    
    var atualizado = [];
    for (var i = 0; i<dados.length; i++){
        if (dados[i].data == removerPela(dados, "regiao","")[dados.length-2].data){
            atualizado.push(dados[i]);
        }
    }
    return atualizado;
}
...
function grafico1ComSp(estado, obitosAcumulados, casosAcumulados){
    ...
    var chart = new Chart(ctx, {
        ...
        options: {
            title: {
                display: true,
                text: ['CASOS E ÓBITOS POR ESTADO','', '**Clique sobre a legenda para interações**'],
            },
            scales: {
                yAxes: [{
                ticks: {
                    min: 0,
                    max: parseInt(casosAcumulados[0])+500,
                    stepSize: 5000
                }
                }]
            }
    ...
    var chart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                ...
                ticks: {
                    min: 0,
                    max: parseInt(casosAcumulados[0])+5000,
                    stepSize: 3000
                }
                }]
            }
        ...
        options: {
            ...
            scales: {
                yAxes: [{
                  ticks: {
                    min: 0,
                    max: parseInt(casosAcumulados[casosAcumulados.length-1])+1000,
                    stepSize: 3000
                  }
                }]
            }
